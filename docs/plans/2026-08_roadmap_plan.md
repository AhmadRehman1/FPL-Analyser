...
**Mid-build corrections (Phase C-1, first build iteration):** building against this plan surfaced two things the interview and initial recon hadn't caught, both fixed in this document before the next phase was built: (1) Track C's original hook point (`data/report_history/`) turned out to be the wrong file on closer reading — it's a generic squad snapshot, not a per-manager hold-vs-transfer decision; corrected to `run_transfer_planner_for_real_squad.py`. (2) A DB-table-based persistence design doesn't survive between `scheduled_pipeline.yml` runs at all (see [A10]/[A11]) — this would have made both Track C and the not-yet-built Track B silently do nothing. Both are the kind of gap the Critique Engine step exists to catch — see Phase C-1's own note and the Risks section below.
...
**Architectural correction (found building Phase C-1, before Track B was built — see [A10]):** `db/fpl_quant_v2.duckdb` is gitignored and does not survive between separate GitHub Actions jobs on its own. Confirmed directly: `scheduled_pipeline.yml` (the twice-daily workflow that actually runs `run_ingestion.py`/`export_track_record.py` — the two files R5 targets) has an `actions/cache/save@v6` step for that path but **no matching restore step**, so every one of its own runs starts from a freshly-seeded DB; `weekly_backtest.yml` (where Phase B-3 was going to wire promotion) only *restores* that cache, and never saves it back. A promotion written only to a DB table during `weekly_backtest.yml`'s Sunday run would therefore be invisible to every `scheduled_pipeline.yml` run afterward — the exact pipeline generating every real recommendation would silently revert to the original hardcoded defaults within hours, defeating Phase B entirely without any error or warning. The active-version state must be a **git-committed file**, reloaded by `run_ingestion.py` at the start of every run, exactly like the project's own existing `data/recalibration/` seed-file mechanism (`write_recalibration_seed_file()`/`load_confirmed_recalibration_seeds()`, already reloaded at `run_ingestion.py:97-119`) — not a DB table alone. The DB table in [A3] below still exists, but only as a within-run convenience/audit view, rebuilt from the committed file every run.
...
- **[A3] Active-version source of truth: a git-committed file (extending `data/recalibration/`'s existing seed mechanism — per [A10] above), mirrored into a within-run DB table `active_param_versions(family, key, version, promoted_at, promoted_by)` for query convenience.** Executor's choice of exact file/column shape; the requirement is that `run_ingestion.py` reloads the committed file into the DB on every run (same as it already does for confirmed recalibration seeds), not that the DB table is itself durable.
- **[A10] The committed active-version file, not the DB table, is the actual source of truth** — see the Approach note above. Any promotion (Phase B-2) must write this file (and commit it, the same way `weekly_backtest.yml` already commits `data/recalibration/` and `data/dashboard/app_track_record.json`), or the promotion has no real effect on live recommendations.
- Reversibility: because `params.py`'s versions are immutable and append-only (verified: `params.py:28-119`), "rollback" is just re-pointing the committed active-version file at a prior version and re-committing it — no data is ever deleted, so this is cheap and safe to implement.
...
A new git-committed file (e.g. `data/recalibration/active_versions.json`, alongside the existing seed files there) recording the active version per `(family, key)`, per [A3]/[A10]. Within a single run, `run_ingestion.py` mirrors it into a new additive DB table `active_param_versions` for convenience. Also adds a `status='auto_confirmed'` value alongside the existing `recalibration_proposals.status` values (verified table: `schema/0008_m7_backtest.sql:61-77`). No destructive change.
...
- [ ] **Phase B-1: Single source of truth for active parameter versions** *(risky — per [A10], getting this wrong means Phase B silently does nothing; run the Critique Engine at build time)*
  Done when: `run_ingestion.py` loads the committed active-version file (per [A3]) at startup and mirrors it into `active_param_versions`, the same way it already loads `data/recalibration/`'s confirmed seeds (`run_ingestion.py:97-119`); `run_ingestion.py` and `export_track_record.py` read from that table instead of hardcoded literals; behavior is unchanged because the file is seeded with today's literal values; full existing test suite still passes; **and** a test simulates a fresh checkout with no cached DB (deletes any local `db/fpl_quant_v2.duckdb`, runs `run_ingestion.py`, confirms the active version still matches the committed file) to directly prove [A10]'s failure mode is closed, not just that the table exists.
  Steps: write the committed active-version file format; `resolve_active_version()` helper in `params.py` that reads the DB table (which itself is populated from the file at startup, not queried cold); replace hardcoded call sites; seed both the file and the table with today's literal values; add the fresh-checkout test; run the full suite.
  Covers: R5; checks: [A10]
- [ ] **Phase B-2: Regression-gated automated promotion** *(risky — run the Critique Engine at build time)*
  Done when: a new function evaluates each pending `recalibration_proposals` row against the active version's own backtest metrics and either promotes (updates the committed active-version file *and* `active_param_versions`, marks `auto_confirmed`) or leaves it `pending` with a logged, machine-readable reason; unit tests cover both the promote and the block path using [A2]'s threshold; a promote-path test confirms the committed file itself changed, not just the in-run DB table.
  Steps: implement the comparison; write the committed-file update; extend status handling; log every decision; tests for both paths.
  Covers: R6; checks: [A2], [A10]
- [ ] **Phase B-3: Wire into the weekly cadence + reversibility** *(risky)*
  Done when: `weekly_backtest.yml`'s existing Sunday run calls Phase B-2's function right after `recalibrate()` **and commits the updated active-version file** (extending the same commit step that already commits `data/recalibration/`/`app_track_record.json` — a promotion that isn't committed is invisible to every `scheduled_pipeline.yml` run afterward, per [A10]); a documented rollback command re-points the committed file (not just the DB) at a prior version and is tested end-to-end, confirming it restores prior behavior exactly across a simulated fresh checkout.
  Steps: workflow edit; extend the commit step to include the active-version file; write/document the rollback command (extends `review_recalibration.py`'s CLI shape) so it edits the committed file; dry-run by invoking `scripts/run_backtest.py` followed by Phase B-2's promotion function directly via CLI against a scratch copy of the DuckDB file plus a scratch copy of the committed file — the same way `tests/test_backtest.py` already exercises `recalibrate()`, not by running the YAML workflow through a GitHub Actions emulator; verify rollback survives a fresh checkout.
  Covers: R7; checks: [A3], [A10]
...
- **[A12] blocking Phase B-1 specifically:** which of these three shapes to build — (i) full sweep, updating all ~12 files in one reviewed change so every script is guaranteed consistent; (ii) narrow Phase B-1 to just the orchestration scripts that run in `scheduled_pipeline.yml`/`weekly_backtest.yml` (`run_ingestion.py`, `export_track_record.py`, `run_transfer_planner_for_real_squad.py`, `track_elite.py`), leaving the remaining ad-hoc/manual scripts (`explain_my_move.py`, `grade_squad.py`, `run_scenarios.py`, `print_chip_timing_roadmap.py`, `run_transfer_planner.py`, `export_leaderboard.py`, `export_projections.py`, `run_backtest.py`, `run_season_simulation.py`, `run_report.py`) on their current hardcoded literals as an explicit, named follow-up; or (iii) given the true cost, reconsider automating only the *run* (my original recommendation) rather than *confirm* too. This is exactly the kind of scope call the plan's own danger-rule reserves for the human, given Track B is already the single highest-blast-radius track in this document — raised rather than decided silently.
